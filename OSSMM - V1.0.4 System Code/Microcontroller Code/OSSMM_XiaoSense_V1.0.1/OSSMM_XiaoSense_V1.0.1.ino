...
//BLECharacteristic Power = BLECharacteristic(OSSMM_POW_UUID);           // Characteristic ID for Sleep Modulator (In Version 1 Vibration Disc), generated with random UUID generator (not needed since DEEP SLEEP is enabled on BLE disconnect)
...
void setup() {
  ...
  */
  ...
  //IMU.setGyroDataRate(LSM6DS_RATE_416_HZ);                      // Set Gyroscope Data Rate, Options: 12.5, 26, 52, 104, 208, 416 Hz, StandardL 104 Hz
  ...
  */
  ...
  // Properties = Notify, Permission = Open
  SleepData.setProperties(CHR_PROPS_NOTIFY);                 // Set characteristic to NOTIFY
  SleepData.setPermission(SECMODE_OPEN, SECMODE_NO_ACCESS);  // Read is open, Write is closed
  ...
  // Configure the SLEEP MODULATOR Characteristic (here the vibration motor)
  // Properties = Read + Write, Permission = Open to read, Open to write, Fixed Len  = 1 (LED state)
  SleepModulator.setProperties(CHR_PROPS_READ | CHR_PROPS_WRITE);
  SleepModulator.setPermission(SECMODE_OPEN, SECMODE_OPEN);
